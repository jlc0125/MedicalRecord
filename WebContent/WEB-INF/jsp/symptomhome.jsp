			...
			function page(str){
				...
				$.get(url, function(data, status){
					...
					for(var i=0; i<data.length; i++){
						txt += '<li><form method="post" action="symptom" target="_blank"><input type="hidden" name="q" value="' + data[i].name + '"><input type="hidden" name="option" value="0"><a href="#" onclick="javascript:$(this).parent().submit();return false;"><div>' + data[i].name + '</div><div>' + data[i].pinci + '</div></a></form></li>';
					}
					if(txt != ""){
...
<body>
	...
	<div class="bg_heise">
		<div class="imgFrame">
			<a href="analysishome"><img src="${initParam.resources}/resources/recipe/image/r_125.png" /></a>
		</div>
				...
				<div class="checkboxes">搜索选项：&nbsp; 
					<c:choose>
						<c:when test="${option == 1}">
							<input type="radio" name="option" value="0">疾病&nbsp;&nbsp;
							<input type="radio" name="option" value="1"   checked>中药&nbsp;&nbsp;
							<input type="radio" name="option" value="2">方剂&nbsp;&nbsp;
						</c:when>
						<c:when test="${option == 2}">
							<input type="radio" name="option" value="0">疾病&nbsp;&nbsp;
							<input type="radio" name="option" value="1">中药&nbsp;&nbsp;
							<input type="radio" name="option" value="2"  checked>方剂&nbsp;&nbsp;
						</c:when>
						<c:otherwise>
							<input type="radio" name="option" value="0" checked>疾病&nbsp;&nbsp;
							<input type="radio" name="option" value="1">中药&nbsp;&nbsp;
							<input type="radio" name="option" value="2">方剂&nbsp;&nbsp;
						</c:otherwise>
					</c:choose>
					<c:if test="${error == 1}">
	  					<div class="errinfo">抱歉，没有找到相关的疾病，请重新输入！</div>
					</c:if>
				</div>
			</form>
		</div>
		...
		<c:when test="${option == 1}">
			<div class="relate_recipe">
				<div class="relate_wrap">
					<div class="relate_info">相关疾病</div>
					<c:choose>
						<c:when test="${noResult == 1}">
							<div class="noResultInfo">抱歉，没有找到相关的疾病，请重新输入！</div>
						</c:when>
						<c:otherwise>
								...
								<tbody>
									<c:forEach var="node" items="${nodes}">
										<tr>
											<td>${node.index}</td>
											<td>
												<form method="post" action="symptom" target="_blank">
