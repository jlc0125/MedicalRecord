...
<head>
	...
	<link rel="stylesheet" type="text/css" href="${initParam.resources}/resources/recipe/css/home.css">
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			function page(str){
				var url = "drug/pinyin?q=" + str;
				$.get(url, function(data, status){
					var txt = "";
					for(var i=0; i<data.length; i++){
						txt += '<li><form method="post" action="drug" target="_blank"><input type="hidden" name="q" value="' + data[i].name + '"><input type="hidden" name="option" value="0"><a href="#" onclick="javascript:$(this).parent().submit();return false;"><div>' + data[i].name + '</div><div>' + data[i].pinci + '</div></a></form></li>';
					}
					if(txt != ""){
...
<body>
	...
	<div class="bg_heise">
		<div class="imgFrame">
			<a href="analysishome"><img src="${initParam.resources}/resources/recipe/image/r_124.png" /></a>
		</div>
		
		<div class="logoSearch_L2">
			<form id="search_cf" class="search cf" method="post" action="drug">
				...
				<div class="checkboxes">搜索选项：&nbsp; 
					<c:choose>
						<c:when test="${option == 1}">
							<input type="radio" name="option" value="0">中药&nbsp;&nbsp;
							<input type="radio" name="option" value="1"   checked>疾病&nbsp;&nbsp;
							<input type="radio" name="option" value="2">方剂&nbsp;&nbsp;
						</c:when>
						<c:when test="${option == 2}">
							<input type="radio" name="option" value="0">中药&nbsp;&nbsp;
							<input type="radio" name="option" value="1">疾病&nbsp;&nbsp;
							<input type="radio" name="option" value="2"  checked>方剂&nbsp;&nbsp;
						</c:when>
						<c:otherwise>
							<input type="radio" name="option" value="0" checked>中药&nbsp;&nbsp;
							<input type="radio" name="option" value="1">疾病&nbsp;&nbsp;
							<input type="radio" name="option" value="2">方剂&nbsp;&nbsp;
						</c:otherwise>
					</c:choose>
					<c:if test="${error == 1}">
	  					<div class="errinfo">抱歉，没有找到相关的中药，请重新输入！</div>
					</c:if>
				</div>
			</form>
		</div>
		...
		<c:when test="${option == 1}">
			<div class="relate_recipe">
				<div class="relate_wrap">
					<div class="relate_info">相关中药</div>
					<c:choose>
						<c:when test="${noResult == 1}">
							<div class="noResultInfo">抱歉，没有找到相关的中药，请重新输入！</div>
						</c:when>
						<c:otherwise>
								...
								<tbody>
									<c:forEach var="node" items="${nodes}">
										<tr>
											<td>${node.index}</td>
											<td>
												<form method="post" action="drug" target="_blank">
