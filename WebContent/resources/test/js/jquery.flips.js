...
(function( window, undefined ) {
	...
	$.Flips.prototype 	= {
		_init 				: function( options ) {
			...
			this.pagesCount		= this.options.pagesCount;
		...
		_adjustLayout		: function( page ) {
			...
			this.$flipPages.each( function( i ) {
				...
				if( _self.nextPage + 2 * i === page * 2 - 1 ) {
				...
				else if( _self.nextPage + 2 * i < 2 * page ) {
		...
		_layout				: function() {
			...
			for( var i = this.nextPage; i < this.nextPage + this.batchSize; i+=2 ) {
				...
				var page2 = "00000000" + (i + 2);
					page2 = page2.substring(page2.length-8, page2.length);
				...
				if(i == 0){
					...
					pageData	= {
						...
						theContentStyleBack		: 'width:600px;'
					};
		...
		_initTouchSwipe		: function() {
			...
			this.$el.swipe( {
				...
				swipeStatus			: function( event, phase, start, end, direction, distance ) {
					...
					// console.log("~~~~~~~~~~~~~~~~~")
					// console.log(_self._isAnimating())
					...
					if( _self.currentPage === 0 && _self.flipSide === 'l2r' || _self.currentPage === _self.pagesCount && _self.flipSide === 'r2l' ) {
		...
		_setFlippingPage	: function() {
			...
			( this.flipSide === 'l2r' ) ?
				...
				this.$flippingPage 	= $(".page-" + (this.currentPage * 2 - 1)) :
				this.$flippingPage	= $(".page-" + (this.currentPage * 2 + 1)) ;
		...
		_onEndFlip			: function( $page ) {
			...
			if( ( this.flipSide === 'l2r' && $page.data( 'flip' ) ) || 
				...
				// console.log(this.pagesCount - $page.index());
				// console.log("=========================");
				// console.log((Number($page.attr("class").split(" ")[1].split("-")[1])-1)/2);
				$page.css( 'z-index', this.pagesCount - (Number($page.attr("class").split(" ")[1].split("-")[1])-1)/2);
		...
		_turnPage			: function( angle, update ) {
			...
			if( this.flipSide === 'r2l' ) {
				// console.log(this.pagesCount  + idx);
				this.$flippingPage.css( 'z-index', this.pagesCount  + idx );
	...
	$.fn.flips			= function( options ) {
		...
		if ( typeof options === 'string' ) {
			...
			this.each(function() {
				...
				// var instance = $.data( this, 'flips' );
				// if ( !instance ) {
					var tmp = $.data( this, 'flips', new $.Flips( options, this ) );
				// }
			});
